...
static ssize_t my_drv_read (struct file *filp, char __user *buf, size_t size, loff_t *offset)
{
	...
	status = gpiod_get_value(my_dev_gpio[minor]);  /* 逻辑电平值 */
...
//	int gpio_pin;
	...
	gpiod_direction_output(my_dev_gpio[dev_cnt], 0);
...
static struct of_device_id my_dev_match[] = {
	{.compatible = "mywork-led"}, 
	{.compatible = "mywork-beep"}, 
	{.compatible = "mywork-jdq"}, 
	{},
};
